...
angular.module('indexedDB', []).provider '$indexedDB', ->
  ...
  cursorValueMapping = deafultCursorValueMapping = (cursor) ->
    val = cursor.value
    id = cursor.key
  ...
  @setCursorValueMapping = (newMapper = deafultCursorValueMapping) ->
    ...
    class ObjectStore
      ...
      getAll: ->
        ...
        if @store.getAll
          ...
          @_mapCursor defer, cursorValueMapping
        defer.promise
      ...
      eachWhere: (query) ->
        ...
        @_mapCursor(defer, cursorValueMapping, req)
        defer.promise
