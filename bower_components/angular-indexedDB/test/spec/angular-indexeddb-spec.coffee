...
describe "$indexedDB", ->
  ...
  beforeEach ->
    ...
    .upgradeDatabase 3, (event, db, txn) ->
      store = db.createObjectStore "AutoIncrementTestObjects",
        keyPath: null
        autoIncrement: true
  ...
  describe '#openStore', ->
    ...
    describe "objects contain the auto-increment key in $key", ->
      promiseBefore ->
        @subject.openStore "AutoIncrementTestObjects", (store) =>
          store.insert([
            {data: "baz"},
            {data: "boo"}
          ]).then (res) =>
      ...
      itPromises "Has the given key of the object in $key", ->
        @subject.openStore "AutoIncrementTestObjects", (store) =>
          store.each().then (values) =>
            for value in values
              expect(value.$key).not.toBeUndefined()
              expect(@keys.indexOf(value.$key)).not.toBe(-1)

    describe "objects contain the given key in $key", ->
      promiseBefore ->
        @subject.openStore "TestObjects", (store) =>
          store.insert([
            {id: 1, data: "foo"},
      ...
      itPromises "Has the given key of the object in $key", ->
        @subject.openStore "TestObjects", (store) ->
          store.each().then (values) ->
            for value in values
              expect(value.$key).not.toBeUndefined()
              expect(value.$key).toBeEqual(value.id)
